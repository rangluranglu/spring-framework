...
final class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(
		...
		// 第一个实现了BeanDefinitionRegistryPostProcessors的为ConfigurationClassPostProcessor类。
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			// 存取即将要被调用的集合，使用完会clear
			List<BeanDefinitionRegistryPostProcessor> currentRegistryProcessors = new ArrayList<>();

			// First, invoke the BeanDefinitionRegistryPostProcessors that implement PriorityOrdered.
			// 首先，调用实现 PriorityOrdered 的 BeanDefinitionRegistryPostProcessors。
			// 1. 获取实现了BeanDefinitionRegistryProcessors 接口的所有类（由于此处还没有开始包扫描，所以自己定义的BD是无法获取，只能获取到Spring定义的类）
			// 2. 便利
			// 3. 依次实例化
			...
			// 完成扫描Registry // 只有Spring定义的 ConfigurationClassPostProcessor
	...
	public static void registerBeanPostProcessors(
		...
		// 有org.springframework.context.annotation.internalAutowiredAnnotationProcessor
		// Register BeanPostProcessorChecker that logs an info message when
		// a bean is created during BeanPostProcessor instantiation, i.e. when
		// a bean is not eligible for getting processed by all BeanPostProcessors.
		// 注册 BeanPostProcessorChecker，它会在 BeanPostProcessor 实例化期间创建一个 bean 时记录一条信息消息，即当一个 bean 不符合所有 BeanPostProcessors 处理的条件时。
		...
		for (String ppName : postProcessorNames) {
			if (beanFactory.isTypeMatch(ppName, PriorityOrdered.class)) {
				...
				priorityOrderedPostProcessors.add(pp);   // AutowiredAnnotationBeanPostProcessor
